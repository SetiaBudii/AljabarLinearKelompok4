...
public class Matriks {
    ...
    public void keyboardSPL(int n, int m){
        System.out.println();
        for(int i = 1; i <= this.GetBrs(); i++){
            System.out.println("Masukkan Element Matriks A baris ke-" + (i));
            for(int j = 1; j <= this.GetKol(); j++){
                if(j == this.GetKol()){
                    System.out.print("Masukkan nilai B Baris ke-"+(i) +" : ");
                    this.SetElmt(i,j,in.nextDouble());
                } else{
                    System.out.print("Masukkan nilai A kolom[" + (j) + "]: ");
                    this.SetElmt(i,j,in.nextDouble());
                } 
            }
            System.out.println();
            ...
            for (int j = 1; j <= variabel; j++){
                if(solusi[i][j] != 0){
                    if(tulisDulu){
                        ...
                        if((solusi[i][j]*10)%10 == 0) System.out.printf("%s%d", S,j); 
                             else System.out.printf("%s%d", S,j); 
                    } else {
                         if(solusi[i][j]<0){
                            S = "-T";
                        }else{
...
//            }
    ...
    public void bacaKoefKeyboard(double[] konstanta, int n, int m){
        ...
        System.out.println();
        for (int i=1; i<=n; i++){
                    System.out.print("Masukan nilai B baris ke- "+ i + ": ");
                    double x = in.nextDouble();
                    konstanta[i] = x;
        }
    }
